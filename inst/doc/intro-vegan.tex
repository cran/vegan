...
2.0-0
in R version 2.13.1 (2011-07-08) on \today}
...
Run 1 stress 0.1192693 
... procrustes: rmse 0.0007240573  max resid 0.001887997 
*** Solution reached
\end{Soutput}
\begin{Sinput}
R> ord
...
\begin{Sinput}
R> plot(ord, type = "n")
R> points(ord, display = "sites", cex = 0.8, pch = 21, col = "red", 
+      bg = "yellow")
...
R> points(ord, disp = "sites", pch = 21, col = "red", bg = "yellow", 
+      cex = 1.3)
\end{Sinput}
\end{Schunk}
\begin{SCfigure}
...
\begin{Schunk}
\begin{Sinput}
R> ord.fit <- envfit(ord ~ A1 + Management, data = dune.env, 
+      perm = 1000)
R> ord.fit
\end{Sinput}
\begin{Soutput}
***VECTORS
     ...
     NMDS1   NMDS2   r2  Pr(>r)  
A1 0.99055 0.13712 0.38 0.02098 *
---
...
Model: cca(formula = dune ~ A1 + Management, data = dune.env)
           Df  Chisq      F N.Perm  Pr(>F)   
A1          1 0.1759 1.9761   1099 0.03364 * 
...
\begin{Schunk}
\begin{Sinput}
R> ord <- cca(dune ~ A1 + Management + Condition(Moisture), 
+      data = dune.env)
R> ord
\end{Sinput}
\begin{Soutput}
