...
\caption{\label{tab:scales} Alternative scalings for \textsc{rda} used
  ...
  $\mathrm{const} = \sqrt[4]{(n-1) \sum \lambda_k}$.  Correspoding
  negative scaling in \texttt{vegan} and corresponding positive
  scaling in \texttt{Canoco} divides each species by its standard deviation.  }
\begin{tabular}{lcc}
...
cca(formula = varespec[i, ] ~ Al + K, data = varechem) 
              ...
              Inertia Rank
Total          2.0832     
Constrained    0.2514    2
Unconstrained  1.8318   21
Inertia is mean squared contingency coefficient 
...
Eigenvalues for unconstrained axes:
    CA1     CA2     CA3     CA4     CA5     CA6     CA7     CA8 
0.49889 0.31147 0.19838 0.17323 0.15853 0.11644 0.09503 0.07083 
(Showed only 8 of all 21 unconstrained eigenvalues)
\end{Soutput}
\end{Schunk}
...
observed 2.9590e-14 because site
