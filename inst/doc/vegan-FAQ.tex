...
design decisions I have made in vegan.  The document was made with
\texttt{vegan} version 
1.7-0 
and R version 1.8.1, 2003-11-21.
\end{abstract}
...
varinace in the data.  The other orthonormal matrix $\mathbf{V}$ can
...
\caption{\label{tab:scales} Alternative scalings for \textsc{rda} used
  ...
  $\mathrm{const} = \sqrt[4]{(n-1) \sum \lambda_k}$.  }
\begin{tabular}{lcc}
...
\texttt{Canoco, scaling=2} &
$u_{ik} \sqrt{n} $ &
$v_{jk} \sqrt{n} \sqrt{\lambda_k / \sum \lambda_k} / s_j$ 
\\
\texttt{Canoco, scaling=3} &
$u_{ik} \sqrt{n} \sqrt[4]{\lambda_k / \sum \lambda_k} $ &
$v_{jk} \sqrt{n} \sqrt[4]{\lambda_k / \sum \lambda_k} / s_j $ 
\end{tabular} 
\end{table}
...
with respect to each other.  For \texttt{Canoco} with negative scaling
values, no constant $c$ can be found, but the correction is dependent
on species standard deviations $s_j$, so this alternative does not
define a biplot.
...
cca(formula = varespec[i, ] ~ Al + K, data = varechem) 
              ...
              Inertia Rank
Total          2.0832     
Constrained    0.2491    2
Unconstrained  1.8341   21
Inertia is mean squared contingency coefficient 
...
Eigenvalues for unconstrained axes:
    CA1     CA2     CA3     CA4     CA5     CA6     CA7     CA8 
0.39725 0.31348 0.21552 0.18684 0.17568 0.11795 0.11287 0.08093 
(Showed only 8 of all 21 unconstrained eigenvalues)
\end{Soutput}
\end{Schunk}
...
observed 2.5646e-14 because site
