            ...
            before = NULL, after = NULL) 
{
  ...
  if (!is.null(before)) {
    ...
    for (i in 1:nr) {
      tmp <- veg[i, ] > 0
      veg[i, tmp] <- approx(before, after, veg[i, tmp], rule = 2)$y
    }
  }
  v <- NULL
  if (iweigh) {
  ...
  eig <- 1
  nid <- sum(veg>0)
  cep <- .C("data2hill", as.double(veg), mi=as.integer(nr), n=as.integer(nc),
            nid=as.integer(nid), ibegin=integer(nr), iend=integer(nr),
            idat=integer(nid), qidat=double(nid), PACKAGE="vegan"
            )[c("mi","n","nid","ibegin","iend","idat","qidat")]
  ...
  s1 <- .Fortran("eigy", x = as.double(xeig1), y = as.double(yeig1), 
                 ...
                 adotj = as.double(adotj), PACKAGE="vegan")[c("x", "y", "eig")]
  if (!ira) 
  ...
  CA <- list(rproj = rproj, cproj = cproj, evals = evals, origin = origin, 
       v = v, adotj = adotj, aidot = aidot, iweigh = iweigh, 
       iresc = iresc, ira = ira, mk = mk - 4, short = short, 
       before = before, after = after, call = match.call())
  class(CA) <- "decorana"
  CA
}
